...
node default {
  ...
  File { '/tmp/nosuchfolder/the☃file':
    ensure => present,
  }
}
SITEPP
  ...
  step "Send an rpc_blocking_request to all agents" do
    ...
    agents.each_with_index do |agent|
      step "Check Run Puppet response for #{agent}" do
        ...
        assert_equal('failed', action_result['status'], "Result of pxp-module-puppet run on #{agent} should be 'changed'")
      end
    end
  end
end
